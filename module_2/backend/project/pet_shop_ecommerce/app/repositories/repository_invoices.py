...
from app.infrastructure.database.db_model import Invoices
...
invoice_transactions = Transactions()
...
class InvoicesRepository:
    ...
    def create_full_invoice(self, user_id, cart_id, shipping_address_id, shipping_method, payment_method, products_in_cart, discount):
        invoice_subtotal = invoice_transactions.calculate_invoice_subtotal(products_in_cart)

        # apply discount if exists
        discount_amount = invoice_transactions.calculate_discount(invoice_subtotal, discount)

        # shipping cost based on shipping method
        shipping_cost = invoice_transactions.calculate_shipping_cost(shipping_method)      

        # invoice total, taxes are fixed at 7% in db_model
        invoice_total = invoice_transactions.calculate_invoice_total(invoice_subtotal, discount_amount, shipping_cost)

        # create the invoice
        invoice = self.new_invoice(user_id, cart_id, shipping_address_id, shipping_method, payment_method,
