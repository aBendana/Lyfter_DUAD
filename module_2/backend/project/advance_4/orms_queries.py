...
from sqlalchemy import insert, select, update, bindparam, delete, and_
...
class QueryFunctions:
    ...
    #     stmt = insert(self.table).returning(self.table.c.id, self.table.c.rol).values(data)
    ...
    def single_insert_register(self, data:dict):
        ...
        if hasattr(self.table.c, "role"):
            columns.append(self.table.c.role)
        if columns:
            stmt = insert(self.table).returning(*columns).values(data)
        else:
            raise ValueError("Insert failed: 'id' or 'role' column not found in table definition")
    ...
    def single_insert(self, data: dict):
        ...
        # enums types returning like a string
        formatted_data = {}
    ...
    def select_by_two_values(self, column_a, column_b, value_a, value_b):
        ...
        with self.engine.connect() as conn:
            result = conn.execute(stmt).fetchall()
            print(f"\n\033[92m(ORM)DOUBLE SELECT{result}\033[0m\n")
        return result
