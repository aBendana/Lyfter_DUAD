...
def login():
    ...
    try:
        ...
        # verifying login status successfully or failed
        login_repo.login_status_verification(result)
        ...
        elif role_type.value == 'cb_user':
            return jsonify(message="User: Successful Login!", access_token=access_token, refresh_token=refresh_token)
...
@logins_bp.route("/refresh-token", methods=["POST"])
def refresh_token():
    try:
        token = request.headers.get("Authorization")
        if not token:
