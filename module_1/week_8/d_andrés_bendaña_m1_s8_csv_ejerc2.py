#Andrés Bendaña
#Módulo 1, Semana 8, "CSV", Ejercicio 2
...
def write_csv_file(path, data, headers):
    ...
    with open("video_games_tab.csv", "x", encoding='utf-8') as file:
        writer = csv.DictWriter(file, headers, dialect="excel-tab")
        writer.writeheader()
        writer.writerows(video_games_list)
        
    with open("video_games_tab.csv") as file:
        print("")
        print(file.read())    
...
def main():
    ...
    write_csv_file("video_games_tab.csv", video_games_list, video_games_list[0].keys())
